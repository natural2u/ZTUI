...
#import "ZTScrollPageTopBarItem.h"

#import <objc/runtime.h>

@interface ZTScrollPageController () {

}

@end

@implementation ZTScrollPageController
...
//    CLScrollPageTopBar *pageTopBar = [[CLScrollPageTopBar alloc] initWithFrame:
//            CGRectMake(0, y, self.view.frame.size.width, 40)];
    ZTScrollPageTopBar *pageTopBar = [self createTopBar:CGRectMake(0, y, self.view.frame.size.width, 40)];
    ...
    ZTScrollPageView *pScrollView = [[ZTScrollPageView alloc] initWithFrame:pagingScrollViewFrame];
...
-(ZTScrollPageTopBar *)createTopBar:(CGRect)topBarRect{
    ZTScrollPageTopBar *pageTopBar = [[ZTScrollPageTopBar alloc] initWithFrame:topBarRect];
    return pageTopBar;
}
...
#pragma mark - CLScrollPageTopBar 相关方法 -
...
- (Class)tableView:(UITableView *)tableView cellClassForObject:(id)object {
    return [ZTScrollPageTopBarCell class];
}
...
//    NSString *identifier = NSStringFromClass(cellClass);
    ...
    if ([cell isKindOfClass:[ZTScrollPageTopBarCell class]]) {
        [(ZTScrollPageTopBarCell *) cell setObject:object];
    }
...
- (UIViewController <ZTScrollPageControllerDelegate> *)dequeueReusablePageWithIdentifier:(NSString *)identifier {
    UIViewController <ZTScrollPageControllerDelegate> *page = nil;
...
- (void)toPageWithAnimate:(NSUInteger)index animate:(BOOL)animate {
    ...
    UIViewController <ZTScrollPageControllerDelegate> *currentPage = [self.visiblePages anyObject];
    ...
    if (self.pageGap > 1) {
        //如果页面间隔大于1
        UIViewController <ZTScrollPageControllerDelegate> *page = [self pageForRowAtIndex:index];
...
- (void)tilePages {
    ...
    for (UIViewController <ZTScrollPageControllerDelegate> *page in _visiblePages) {
    ...
    for (int index = firstNeededPageIndex; index <= lastNeededPageIndex; index++) {
        ...
        if (![self isDisplayingPageForIndex:(uint) index]) {
            ...
            UIViewController <ZTScrollPageControllerDelegate> *page = [self dequeueReusablePageWithIdentifier:identifier];
...
- (UIViewController <ZTScrollPageControllerDelegate> *)getDisplayingPageForIndex:(NSUInteger)index {
...
- (void)configurePage:(UIViewController <ZTScrollPageControllerDelegate> *)page forIndex:(NSUInteger)index {
    [page setIndex:index];
...
- (void)callBack {
    ...
    if (self.pageGap > 1) {
        ...
        UIViewController <ZTScrollPageControllerDelegate> *page = _visiblePages.anyObject;
    ...
    UIViewController <ZTScrollPageControllerDelegate> *page = [arr objectAtIndex:0];
